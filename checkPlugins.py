...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Check the pool of available public PluMA plugins.')
    parser.add_argument('--directory', dest='dirPath', action='store',
        default=path.join(path.realpath(path.dirname(__file__)), 'plugins'),
        help='The path to where the downloaded plugins are located.')
    ...
    if (len(sys.argv) > 1):
        ...
        plugins = os.listdir(args.dirPath)
    ...
    for plugin in local.intersection(pool):
        x = os.popen('cd ' + args.dirPath + '/' + plugin + '; git diff; cd ..').read()
        if (len(x) != 0):
