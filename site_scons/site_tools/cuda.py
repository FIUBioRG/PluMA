...
Modified from https://github.com/SCons/scons-contrib/blob/master/sconscontrib/SCons/Tool/cuda/__init__.py
"""
...
def generate(env):
    ...
    # default compiler
    env.Append(NVCC = os.getenv('NVCC', 'nvcc'))

    # default flags for the NVCC compiler
    env.Append(NVCCFLAGS = os.getenv('NVCCFLAGS', ''))
    env.Append(STATICNVCCFLAGS = [os.getenv('STATICNVCCFLAGS', '')])
    env.Append(SHAREDNVCCFLAGS = [os.getenv('SHAREDNVCCFLAGS', '')])
    env.Append(ENABLESHAREDNVCCFLAG = [os.getenv('ENABLESHAREDNVCCFLAG', '-shared')])

    # default NVCC commands
    env.Append(STATICNVCCCMD = ['$NVCC -o $TARGET -c $NVCCFLAGS $STATICNVCCFLAGS $SOURCES'])
    env.Append(SHAREDNVCCCMD = ['$NVCC -o $TARGET -c $NVCCFLAGS $SHAREDNVCCFLAGS $ENABLESHAREDNVCCFLAG $SOURCES'])
    ...
    try:
        cudaToolkitPath = env.subst('$CUDA_TOOLKIT_PATH')
    except:
        paths=[
    ...
    env.Append(CUDA_TOOLKIT_PATH = [cudaToolkitPath])

    # find CUDA SDK path and set CUDA_SDK_PATH
    try:
        cudaSDKPath = env.subst('$CUDA_SDK_PATH')
    except:
        paths = [
    ...
    env.Append(CUDA_SDK_PATH = [cudaSDKPath])

    # cuda libraries
    if env.subst('$PLATFORM') == 'posix':
