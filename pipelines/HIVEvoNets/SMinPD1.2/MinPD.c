...
/*BC: Comments for edits done by Brice Cadwallader start with "BC:"*/
...
#include "Heapsort.h"
#include "mathfuncs.h"
#include "parse_markers.h" /*BC: new routines to support new markers format*/

/*BC: BootscanCandidates prototype and GammDis macro moved to MinPD.h*/
...
int old_opt;
int old_recOn;
/*BC: end variables for preclustering*/
						 ...
						 double min_seq_dist, int align, int bootOpt, int step,int *avgBS)
{
 ...
 //fROC_test=fopen("ROC.txt","a");
		...
		for(i=0;i<win_Count;i++){/* Sort by distance & find largest distance*/
			...
			if(largest<d) /*BC: largest was never initialized*/
				largest=d;
		}
...
BKPNode *Check4Recombination(int  *Frag_Seq,  Fasta **seqs, double **dist, double ***dist_frags, int startAS, 
				...
				int step, int windowSize , short int bootReps, double min_d)
{
	...
	if (i>1){
		if(opt==0) 
			...
			return(BootscanCandidates(seqs,Frag_Seq, step,  windowSize ,  bootReps,i,s,win_Count,min_idx,dist[s][min_idx],align,maxDim,0,&dummy));
	}
	else
		return(NULL);
}
...
/******************************************************************************/
/*  	BC: RNACodonToDNACodon:  return the DNA codon from which an RNA	comes */
/******************************************************************************/
char* RNACodonToDNACodon(char* RNACodon){
	int i;
	char* DNACodon;
	if( (DNACodon = (char*)malloc(3 * sizeof(char))) == NULL){
		ErrorMessageExit("Memory error in RNACodonToDNACodon", 1);
	}
	for(i = 0; i < 3; i++){
		switch(RNACodon[i]){
			case 'A':
			...
			case '-':
				...
				break;
			default:
				ErrorMessageExit("Unexpected character found in RNACodonToDNACodon\n", 1);
		}
	}
	return DNACodon;
}
/***************************************************************************/
/*  	 BC: CodonToAminoAcid:  return the amino acid abbreviation (char)  */
/*       which corresponds to the three character triplet whose first      */
/*       character is pointed to by Codon (char*)					       */
/***************************************************************************/
char CodonToAminoAcid(char* Codon){
	int i;
	if(RNA){
		Codon = RNACodonToDNACodon(Codon);
	}
	for(i = 0; i < NumberOfTriplets; i++){
		if(strncmp(Codon, DNATriplets[i], 3) == 0){
			if(RNA){
				free(Codon);
			}
			return DNAAminoAcids[i];
		}
	}
	return 0;
}
...
{
	int i,j,k,c,p;
	char AminoAcid1, AminoAcid2;
	fflush( stdout );
	/*BC: keep record of sequences kept in cluster*/
	fprintf( ClusterFile, "%s", seqs[s]->Id );
	fflush( stdout );
	for (i=0;i<max_times;i++){
		if( clustBoot == 0 ){
			break;
		...
		/* check if identical at condon positions*/
			...
			for(j=0;j<clustBoot;j++){
				c=codonList[j];
					/*BC: clustering by amino acid*/
				if( Cluster == 1 || Cluster == 4 ){
					//printf("\tChecking AA of %s at %d...\n", seqs[mintaxa[i].Anc_Idx]->Id, c);/*BC:debug*/
					fflush( stdout );
					AminoAcid1 = CodonToAminoAcid(&seqs[k]->Seq[c]);
					AminoAcid2 = CodonToAminoAcid(&seqs[min_idx]->Seq[c]);
					if(AminoAcid1 == 0 || AminoAcid2 == 0){
						break; /*consider them not identical if either of them is ambiguous (ie not found in the lookup table)*/
					}
			...
			if(j<clustBoot){
				...
				//printf("\tRemoving %s\n", seqs[mintaxa[i].Anc_Idx]->Id);
				fprintf(ClusterFile, ",[%s]", seqs[mintaxa[i].Anc_Idx]->Id);
		...
		/*BC: keep record of sequences kept in the cluster*/
			else{
				//seqs[mintaxa[i].Anc_Idx]->Cluster = 1;
				fprintf(ClusterFile, ",%s", seqs[mintaxa[i].Anc_Idx]->Id);
			}
		}
	/*BC: if we're looking at the ancestor at minimum distance, record that as part of the cluster*/
		else{
			fprintf( ClusterFile, ",(%s)", seqs[mintaxa[i].Anc_Idx]->Id );
			seqs[mintaxa[i].Anc_Idx]->Cluster = 1;
		}
		...
		if (bkpLL!=NULL){/*Is recombinant */
			...
			while(bkpLL!=NULL){
				...
				if(SaveMinResults(minresults, resno, bkpLL->Child_Idx,s,dist[s][bkpLL->Child_Idx],avgBS)==false){
					//printf("saveminresults==false\n");
					return(false);
				}
				else{
					//printf("saveminresults==true\n");
				}
			...
			t=startAS;
			if(clustBoot>0)
				interval=(threshold)*(maxDim/(double)1000);
//int *codonList;
			else
			...
			/*BC: if there is only one candidate ancestor sequence, still keep track of it as a cluster of one*/
			if( clustBoot > 0 && times == 1){
				//printf("debug Recording cluster of one:\n");
				//printf("debug \t%s,%s\n", seqs[s]->Id, seqs[mintaxa[0].Anc_Idx]->Id);
				if( PreCluster == 1 ){
					fprintf( ClusterFile, "%s,(%s)\n", seqs[s]->Id, seqs[mintaxa[0].Anc_Idx]->Id );
					fflush( ClusterFile );
				}
				else if( PreCluster == 0 && Cluster > 3 ){
					fprintf( ClusterFile, "%s,%s\n", seqs[s]->Id, seqs[mintaxa[0].Anc_Idx]->Id);
					fflush( ClusterFile );
				}
				//printf( "debug done recording clsuter of one\n" );
			}
			...
			if( clustBoot > 0 && times > 1 ){ 
				...
				if( PreCluster == 1 && Cluster != 3 ){
					//printf( " checking clustering.....\n" );
					times = CheckClustering( mintaxa, times, seqs, s, min_idx );
					//printf( "done checking clustering\n" );
				}
				else if( PreCluster == 0 && Cluster > 3 && Cluster < 6 ){
					//printf( " checking clustering.....\n" );
					times = CheckClustering( mintaxa, times, seqs, s, min_idx );
					//printf( "done checking clustering\n" );
				}
			...
			//printf( "debug here8\n" );
			if( PreCluster  == 1 ){
				seqs[min_idx]->Ancestor = 1;
				for( i = 0; i < times; i++ ){
					seqs[mintaxa[i].Anc_Idx]->Cluster = 1;
					if( dist[mintaxa[i].Anc_Idx][min_idx] >= interval ){
						seqs[mintaxa[i].Anc_Idx]->Very_Different = 1;
					}
				}
			}
			else if( PreCluster == 0  && Cluster > 3 ){
			...
			if( PreCluster != 1 ){
				...
				/* Save in struct */
				if(SaveMinResults(minresults, resno, mintaxa[j].Anc_Idx,s,mintaxa[j].dist, mintaxa[j].div)==false){
					return (false);
				}
			}
			//printf( "debug 10\n" );	
	...
	*/
	...
	if( PreCluster != 1 ){
		if ((fp2 = (FILE *) fopen(File2,"w")) == NULL) {
			printf("io2: error when opening distances files\n");
				return(1);
		}
	...
	/* Then write distances to file */
	 for (s;s<=n;s++){
		...
		for(t;t>=0;t--){
			...
			if(reportDistances && PreCluster != 1 )
		...
		if (win_Count>1 && recOn) /* Recombination Detection */
			bkpLL = Check4Recombination(Frag_Seq,seqs,dist,
		...
		//	printf("done with output results\n" );
		}
...
//fprintf(stderr,";\n\n");
	...
	while(r<resno){
		...
		if(ch==2){
			...
			NJTree(NodeStorage, DistMatrix, ch, false, children, &max_w, &w); /*BC changed true to false*/
			//print_nodes( NodeStorage[w-1], 0, seqs );
			new_root = Reroot( NodeStorage[w-1], new_root_index, 1, seqs ); /*BC reroot the tree*/
			PrintTree(fp1,new_root,&seqs[0],&boots[0],printBoot);
			fprintf(fp1,";\n\n");
...
/******************************************************************************************/
 ...
 int BootscanShrinkPool(Fasta **seqs, int  *Frag_Seq, int step, int windowSize , short int bootReps,
...
BKPNode *BootscanCandidates(Fasta **seqs, int  *Frag_Seq, int step, int windowSize , short int bootReps,
	...
	int writeMatrix;
	 ...
	 if((mintaxa = (Mintaxa *) malloc(sizeof(Mintaxa) * 20))==NULL)
		...
		if ((fp_d = (FILE *) fopen(File2,"w")) == NULL) 
			ErrorMessageExit("io2: error when opening distances file\n",1);
...
//		}
		...
		for (s;s<n;s++){
			...
			if(numSeqs>1){
				...
				c=BootscanShrinkPool(seqs,Frag_Seq,step,windowSize,bootreps,numSeqs,s,numWins,maxDim,100-(numSeqs*5),60/step);
				if(c>1){
					//PrintFragments(fp3,Frag_Seq,seqs, dist, dist_frags, numWins,c, s , align);
				}
				if(c>1){
					bkpLL=BootscanCandidates(seqs,Frag_Seq, step,  windowSize ,  bootreps,c,s,numWins,min_idx,dist[s][min_idx],align,maxDim,0,&avgBS);
				}
			}
		...
		switch (ch) {
			...
			case 'e':
				if (fscanf(stdin, "%d", &seed)!=1 ) {
					...
					ErrorMessageExit( "Bad [bootscan seed] number\n", 1 );
					PrintUsage();
				}
			break;
			case 'f':
			...
			case 'g': /* BC: option */
				...
				if(!isspace(ch)){
					...
					codonFile[j]='\0';
					if( strlen( codonFile ) == 0 ){
						clustBoot = 0;
						break;
					}
					//printf("codon file is %s\n", codonFile);
					if( parse_markers_file( codonFile, &codonList, &clustBoot ) ){
						fprintf( stderr, "error parsing markers file '%s'\n", codonFile );
						exit( -1 );
					}
					if( DEBUG_MODE ){
						fprintf( stderr, "found %d markers:\n", clustBoot );
						for( counter = 0; counter < clustBoot; counter++ ){
							fprintf( stderr, "\t%d\n", codonList[counter] );
						}
			...
			break;	
			/*BC: use j to toggle clustering options*/
			/*
				[clustering option] j1 
				0:no clustering
				1:by amino acid
				2:by bases
				3:only by distance
				4:like 1 but post-clustering
				5:like 2 but post
				6:like 3 but post
			*/
			case 'j': /* BC: optional */
				...
				if( ( Cluster < 0 ) || ( Cluster > 6 ) ){
						fprintf(stderr, "valid cluster values are 1-6 (%d and %s)\n\n", Cluster, digit);
						PrintUsage();
				}
			break;
			case 'k':
			...
			break;
			case 'O': /* BC: output directory option */
				ch=fgetc(stdin);
				if(!isspace(ch)){
					j=0;
					if(ch=='"'){
						ch=fgetc(stdin);
						while(ch!='"'){
							output_directory[j]=ch;
							j++;
							ch=fgetc(stdin);
						}
					}
					else{
						fprintf(stderr, "output directory needs to be enclosed in quotation marks\n\n");
						PrintUsage();
					}
					output_directory[j]='\0';
					if( strlen( output_directory ) != 0 && strrchr( output_directory, '/' ) == NULL && strrchr( output_directory, '\\' ) == NULL ){
						fprintf(stderr, "output directory file needs to be an absolute path\n\n");
						PrintUsage();
					}
					if( j != 0 && output_directory[j-1] != '/' && output_directory[j-1] != '\\' ){
						output_directory[j] = '/';
						output_directory[j+1] = '\0';
					}
				}
			...
			break;
			case 'T': /* BC 'T' for threshold (optional) */
				if( fscanf( stdin, "%lf", &threshold ) != 1 ){
					fprintf( stderr, "bad codon clustering threshold value\n" );
					PrintUsage();				
				}
				else{
					if( DEBUG_MODE ){
						fprintf( stderr, "got codon clustering threshold value %g\n", threshold );
					}
				}
			break;
			case 'v':
...
int i, time;
time=seqs[0]->time;
	for (i=1;i<=n;i++){ /*BC: This was changed to be i <= n  instead of i < n so that all of the sequences are checked*/
		if(time!=seqs[i]->time){
				break;
		}
	}
	if(i>n){ /*BC: This was changed to be i > n i == n*/
...
int main(int argc, char* argv[])
{
	...
	if( Cluster != 0 && clustBoot == 0 ){
		fprintf( stderr, "warning, no clustering will be done because no codons were found\n" );
		Cluster = 0;
	}

	//printf("'%s'\n", OutputFileName );
	output_file = OutputFileName[0] == '\0' ? 0 : 1;
	//printf("output_file is %d\n", output_file);
	ch = strrchr( output_directory, '/' );
	ch2 = strrchr( output_directory, '\\' );
	if( ch != NULL && ch2 != NULL ){
		ch = ch > ch2 ? ch : ch2;
	}
	else if( ch != NULL || ch2 != NULL ){
		ch = ch == NULL ? ch2 : ch;
	}
	if( ch != NULL ){
		*ch = '/';
		*(ch + 1) = '\0';
		strcat( ClusterFileName, output_directory );
		strcat( DistanceFileName, output_directory );
		if( output_file == 0 ){
			strcat( OutputFileName, output_directory );
		}
	}
	ch = strrchr( InputFileName, '/' );
	ch2 = strrchr( InputFileName, '\\' );
	if( ch != NULL && ch2 != NULL ){
		ch = ch > ch2 ? ch : ch2;
	}
	else if( ch != NULL || ch2 != NULL ){
	...
	if( ch != NULL ){
		if( output_file == 0 ){
			strcat( OutputFileName, ( ch + 1 ) );
		}
		strcat( ClusterFileName, ( ch + 1 ) );
		strcat( DistanceFileName, ( ch + 1 ) );
	}
	else{
		if( output_file == 0 ){
			strcat( OutputFileName, InputFileName );
		}
		strcat( ClusterFileName, InputFileName );
		strcat( DistanceFileName, InputFileName );
	}
	if( output_file == 0 ){
		strcat( OutputFileName, Transitions_Suffix );
	}
	strcat( ClusterFileName, Cluster_Suffix );
	strcat( DistanceFileName, Distances_Suffix );
	...
	/*open input file*/
	if ((fp1 = (FILE *) fopen(InputFileName, "r")) == NULL) {
	...
	do{
		...
		if (temp[0]=='>'){
			...
			timechar[1]=temp[i+1];
			timechar[2]=temp[i+2];
			timechar[3]='\0'; /*BC: this is why timechar should be of length 4*/
	...
	if( 1 <= Cluster && Cluster <= 3 ){
		...
		temp_seqs = (Fasta **)malloc( sizeof( Fasta* ) * (n+1) );

		/*open PreCluster output file file*/
		if ((ClusterFile = (FILE *) fopen(ClusterFileName, "w")) == NULL) {
			printf("io: error when opening file:%s.\n%s\n",ClusterFileName,strerror (errno));
			return(1);
		}
		...
		if( GetMinDist(minresults, seqs,dist,dist_frags, maxDim, n,OutputFileName,DistanceFileName,mutrate,fr_Dim, &res_no, align,crossOpt,stSize,wSize)!=true){
		...
		for( i = 0; i <= old_n; i++ ){
			if( seqs[i]->Cluster == 0 || seqs[i]->Very_Different == 1 || seqs[i]->Ancestor == 1 ){
				temp_seqs[++n] = seqs[i];
			}
			else{
				//fprintf( keep_PreClusterFile, "removing %s\n", seqs[i]->Id );
				free( seqs[i] );
			}
		}
		for( i = 0; i <= n; i++ ){
	...
	if( ( clustBoot > 0 ) && ( Cluster >= 4 ) ){
		//fprintf( stderr, "opening %s\n", ClusterFileName );
		ClusterFile = fopen(ClusterFileName, "w");
		if(ClusterFile == NULL){
			ErrorMessageExit("Couldn't open cluster file for writing.", 1);
		}
	}
	...
	if(opt<2){
		...
		fflush(stdout);
		res_no=DistOnly(seqs, dist, dist_frags,  maxDim, n,  fr_Dim, model,wSize,stSize,numWins);
		printf("done with pairwise distances...\n");
	}
	if(res_no==0){
			 fprintf(stderr,"ERROR calling Align or Distance Calculation functions!\n");
			 return(false);
	...
	if(opt>1){/*Sliding Window analysis*/
		fprintf( stdout, "Sliding Window analysis...\n");
		fflush(stdout);
		//res_no=BootscanTest(minresults,seqs,dist,maxDim,n,stSize,wSize,0,&res_no,PCCThreshold);
		if( Bootscan(OutputFileName,DistanceFileName,dist_frags,numWins,minresults, seqs, dist, maxDim, n+1,  stSize, wSize,closestRel,&res_no,bootreps,align)!=true){
		...
		if(GetMinDist(minresults, seqs,dist, 
			dist_frags, maxDim, n,OutputFileName,DistanceFileName,
			mutrate,fr_Dim, &res_no, align,crossOpt,stSize,wSize)!=true){
			 fprintf(stderr,"ERROR calling GetMinDist function!\n");
			 return(false);
		}
	}
	BuildPartialNJTrees(seqs,minresults, res_no, dist,OutputFileName,n);

	/*BC: close ClusterFile if it's opened*/
	if(ClusterFile != NULL){
		fclose(ClusterFile);
	}
